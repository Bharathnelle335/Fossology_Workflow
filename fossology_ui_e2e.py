...
def find_recent_run(workflow_file: str, created_after: datetime):
    ...
    for run in runs:
        created_at = datetime.fromisoformat(run.get("created_at").replace("Z", "+00:00")).astimezone(timezone.utc)
        created_after_utc = created_after.replace(tzinfo=timezone.utc) if created_after.tzinfo is None else created_after.astimezone(timezone.utc)
...
if run_clicked:
    if not gh_ok():
        ...
        if r.status_code in (201, 204):
            ...
            st.session_state["dispatch_time"] = datetime.now(timezone.utc)
